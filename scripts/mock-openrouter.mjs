...
function jsonReply(system, user) {
  ...
  if (/maintain the scene state/i.test(system)) {
    const u = user.toLowerCase();
    if (u.includes('climb') || u.includes('tower')) return { locationChanged: true, location: 'The Church Tower', relativeTo: 'The Drowned Bell', direction: 'n', distance: 'near', description: 'A crooked stone tower above the town', situation: 'Claw marks score the stairs; the bell tolls above.', timeOfDay: null, weather: null, present: [] };
    return { locationChanged: false, situation: 'The mist thickens outside the tavern.', timeOfDay: null, weather: null, present: ['Marla Voss'] };
  }
  if (/Write ONLY the "/i.test(system)) return { value: 'A fresh take written for this single field, dry and specific.' };
